...
import (
	"github.com/Hertucktor/archive-api/config"
	"github.com/Hertucktor/archive-api/mongodb"
...
func createNewCardEntryOnAllCardCollection(c *gin.Context, logger *zap.SugaredLogger) {
	...
	if err != nil {
		...
		log.Fatalf("Couldn't receive card set:%v with number:%v%v", requestBody.SetName, requestBody.Number, err)
		return
	}
	...
	defer func() {
		if err = client.Disconnect(ctx); err != nil {
			logger.Fatalf("Fatal: closing client\n%v", err)
		}
		cancelCtx()
	}()
}
